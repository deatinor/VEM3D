...
/** Specialized class for 2D Mesh that inherits from Mesh
 *
...
void Mesh2D<real>::setAnything2DMesh(string connection) {
	...
	while (getline(file2, fileRow2)) {
		...
		vector<shared_ptr<MeshPoint<2,real>>> vertexPointerVector; // vector of pointer to vertices of the Polygon
		...
		// Create the vertexPointerVector
		for (int i=0; i<numberOfVertexes; i++) {
...
void Mesh2D<real>::setBoundaryElements() {
	// Reorder pairVector to faster compare the edges
	sort(pairVector.begin(),pairVector.end(),lessPair);
	
	// Set all the points with isBoundary=false
	for (int i=0;i<this->pointVector.size();i++) {
		this->pointVector[i]->setIsBoundary(false);
	}
	
	// Compare the pairs
	for (int i=0;i<pairVector.size()-1;i++) { // Exclude the last pair
		if (pairVector[i].first!=pairVector[i+1].first || pairVector[i].second!=pairVector[i+1].second) {
			...
			i++;
			// if same pair, it jumps to the next pair
		}
	}
...
void Mesh2D<real>::setRemainingThings() {
	// set the pointIDs and the last properties

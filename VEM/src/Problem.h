...
/** Virtual class to represent a very generic numerical problem.
 *
 *	The idea is to provide a general set of methods common to a large class of numerical problems.
 ...
 *	
 *	The children classes have to override the methods computeStiffnessMatrix() and computeKnownTerm().
 *
 *	It uses Eigen for store all the matrices and to solve the linear system.
 *
...
class Problem {
	...
	public:
		 ...
		 *
		 *	It allocates the space for all the matrices
		 */
		...
		/** Method to compute the solution. It solves the linear system.
		 *
		...
		virtual void displayError(muParserInterface<embedded,real>& realSolutionFunction,string outputError="error.err",string errorAction="append"); //!<  It displays the error after the computation and prints it to a file
		virtual void write(string outputPoints="points.point",string outputConnections="connections.conn",string outputSolution="solution.sol");	//!< Full output to given files
		virtual void writeSolution(string outputSolution="solution.sol"); //!< Output the solution to a file
};
...
// displayError
template <long embedded,typename MeshType,typename real>
void Problem<embedded,MeshType,real>::displayError(muParserInterface<embedded,real>& realSolutionFunction,string outputError,string errorAction) {

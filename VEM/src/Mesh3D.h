 ...
 *
 *	2 possibilities of file type: tetrahedron and anything3D. In the first case, for each Polyhedron, only the vertices are specified in the connection file. In the second case, for each Polyhedron, all the faces are specified.
 *
 *
 */
...
class Mesh3D : public Mesh<3, Polyhedron<3,real>, OPEN, real> {
	...
	public:
		 ...
		 *	It also checks if the Polygon is on the boundary or not. It is faster to do it now, rather than after having read everything.
		 */
		 ...
		 *
		 *	e.g. The following is a Polyhedron, where 10 is the 10th point in the pointVector and each ';' describes a face:
...
shared_ptr<Polygon<3,real>> Mesh3D<real>::newFace(Args ...arguments) {
	...
	for (int i=0; i<point1.numberOfPolygons(); i++) {
		...
		if (auto polygon=weakPolygon.lock()) {
			if (*polygon==*newPolygon && polygon!=newPolygon) { // different pointers but equal polygons
...
void Mesh3D<real>::setAnything3DMesh(string connection) {
	...
	while (getline(file2, rigaFile2)) {
		...
		// for each polyhedron, take the faces
		for (long i=0; i<numberOfFaces; i++) {
			string faceString;
			getline(rigaFileStream, faceString, ';');
			istringstream stream2(faceString);
			
			vector<long> vertexVector; // In this vector is stored the position in pointVector of each vertex of the Polygon to create
			vector<shared_ptr<MeshPoint<3,real>>> vertexPointerVector; // vector of pointer to vertices of the Polygon
			
			// Count the number of vertices for each face
	...
	for (int i=0; i<polygonVector.size(); i++) {
		if (polygonVector[i]->getIsBoundary()) {
			...
			if (auto polyhedron=polygonVector[i]->polyhedron(0).lock()){
				...
				cerr << "Polyhedron is expired" << endl;
			}
		}
	}
}
...
void Mesh3D<real>::setTetrahedronMesh(string connection) {
	...
	while (getline(file2, rigaFile2)) {
		...
		// Obtain vertices of the Polyhedron

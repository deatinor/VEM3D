...
In Figure \ref{img:conv2d} is plotted the absolute a priori error in $H^1(\Omega)$-norm. We can clearly see that asymptotically, the error behaves as $h^2$, which is what one could expect after reading Theorem \ref{conditions} and section \ref{errors}. 
...
\end{tabular}
\end{center}
\caption{Sum up of the convergence rates under h-refinement, on Laplace $2$D problem.}
\end{table}

\noindent Finally, the non asymptotic behaviour is more chaotic since the approximation is very raw with very few elements. \\
...
In Figure \ref{img:conv3d} is plotted the absolute a priori error in $H^1(\Omega)$-norm. We can see again that asymptotically, the error behaves as $h^2$, which is again what one could expect after reading Theorem \ref{conditions} and section \ref{errors}. To see it in a clearer way, we should refine the mesh even more, but a lot of RAM is needed to solve the problem ($>32$GB). Indeed, with $h=128$, we already take into account $128^3>2$ million degrees of freedom. 
...
\end{tabular}
\end{center}
\caption{Sum up of the convergence rates under h-refinement, on Laplace $3$D problem.}
\end{table}

Note that this $h^2$-component of the error is a typical characteristic of all finite element-type methods: the classical finite element methods \cite{quarteroni}, the spectral element methods \cite{chqz1}, the isogeometric analysis \cite{igaconv} and here the virtual element method.